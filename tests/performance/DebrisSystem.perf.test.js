...
describe('Performance Benchmarks', () => {
    ...
    describe('Debris System', () => {
        ...
        test('penalty (Iter)', () => {
            ...
            const start = performance.now();
            for (let i = 0; i < N_PENALTY; i++) {
                // Manually call penalty calc to stress test iteration
                pet.recalculateCleanlinessPenalty();
            }
            console.log(`Debris Penalty ${N_PENALTY}: ${(performance.now() - start).toFixed(2)}ms`);
        });
    ...
    describe('updateDominantArchetype', () => {
        ...
        test('Benchmark updateDominantArchetype', () => {
            ...
            for (let i = 0; i < iterations; i++) {
                scenarios[i % numScenarios]();
                // We'd need to actually call updateDominantArchetype on a real instance or re-implement
                // Since this is a perf test suite, we'll keep it simple or use a real pet if needed.
                // For now, let's just use the debris tests as they are the primary focus.
            }
