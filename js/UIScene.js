...
export class UIScene extends Phaser.Scene {
  ...
  startTutorial() {
    ...
    runTutorialSequence() {
        ...
        overlay = this.add.container(0, 0).setDepth(2000);
        let dim = this.add.rectangle(0, 0, width, height, 0x000000, 0.7).setOrigin(0).setInteractive();
        overlay.add(dim);

        let msgBox = this.add.container(width / 2, height / 2);
        let msgBg = this.add.rectangle(0, 0, 400, 150, 0x333333).setStrokeStyle(4, 0x4CAF50);
        let msgText = this.add.text(0, -10, "", { fontFamily: 'VT323', fontSize: '24px', color: '#fff', align: 'center', wordWrap: { width: 350 } }).setOrigin(0.5);
        let hintText = this.add.text(0, 40, "(Click anywhere to continue)", { fontFamily: 'VT323', fontSize: '18px', color: '#aaa' }).setOrigin(0.5);
        msgBox.add([msgBg, msgText, hintText]);
        overlay.add(msgBox);

        let highlightGraphic = this.add.graphics();
