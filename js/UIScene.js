...
export class UIScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.dashboardBorder = this.add
      .rectangle(0, 0, 1, 1, 0x4a4a4a)
      .setOrigin(0);
    ...
    this.jobBoardButton = ButtonFactory.createButton(
      this,
      0,
      0,
      "Job Board",
    ...
    this.retireButton = this.add
      ...
      .setVisible(false)
      .on("pointerdown", () =>
        this.game.events.emit(EventKeys.UI_ACTION, EventKeys.RETIRE),
      );
    ...
    if (tabId === "CARE")
      ...
      actions = [
        ...
        { text: "Study", action: EventKeys.STUDY },
        {
          ...
          condition: () =>
            this.nadagotchiData && this.nadagotchiData.currentCareer,
      ...
      actions = [
        ...
        { text: "Wiki", action: EventKeys.OPEN_WIKI },
        ...
        { text: "Settings", action: EventKeys.OPEN_SETTINGS },
        {
          ...
          condition: () =>
            this.nadagotchiData && this.nadagotchiData.isLegacyReady,
  ...
  showTab(tabId, force = false) {
    ...
    this.tabButtons.forEach((btn) => {
      btn.setAlpha(btn.tabId === tabId ? 1.0 : 0.7);
    });
  ...
  updateActionButtons(force = false) {
    ...
    const dashboardHeight = Math.floor(
      this.cameras.main.height * Config.UI.DASHBOARD_HEIGHT_RATIO,
    );
      ...
      const btn = ButtonFactory.createButton(
        ...
        },
        {
          ...
          onDisabledClick: () => {
            this.showToast(
              "Action Locked",
              item.disabledMessage || "Not available yet.",
              "🔒",
            );
          },
        },
  ...
  resize(gameSize) {
    ...
    const dashboardHeight = Math.floor(
      height * Config.UI.DASHBOARD_HEIGHT_RATIO,
    );
    ...
    if (this.calendarDropdown)
      this.calendarDropdown.setPosition(width - 160, safeTop);
  ...
  resizeModals(width, height) {
    ...
    this.allModals.forEach((container) => {
      ...
      if (container.closeButton)
        container.closeButton.setPosition(
          modalWidth / 2 - 40,
          -modalHeight / 2 + 30,
        );
  ...
  async handleUIActions(action, data) {
    switch (action) {
      case EventKeys.OPEN_SHOWCASE:
        ...
        break;
      case EventKeys.OPEN_JOURNAL:
        await this.openJournal();
        break;
      case EventKeys.OPEN_RECIPES:
        await this.openRecipeBook();
        break;
      case EventKeys.OPEN_HOBBIES:
        this.openHobbyMenu();
        break;
      ...
      case EventKeys.DECORATE:
        this.openDecorateMenu();
        break;
      case EventKeys.INTERACT_NPC:
        this.openRelationshipMenu();
        break;
      case EventKeys.OPEN_ANCESTOR_MODAL:
        this.openAncestorModal(data);
        break;
      case EventKeys.OPEN_INVENTORY:
        this.openInventoryMenu();
        break;
      case EventKeys.OPEN_ACHIEVEMENTS:
        this.openAchievementsModal();
        break;
      case EventKeys.OPEN_WIKI:
        this.openWikiMenu();
        break;
      case EventKeys.OPEN_SETTINGS:
        this.openSettingsMenu();
        break;
      case EventKeys.OPEN_CAREER_MENU:
        this.openCareerMenu();
        break;
      case EventKeys.OPEN_JOB_BOARD:
        this.openJobBoardMenu();
        break;
    }
  }
  ...
  getMoodEmoji(mood) {
    return (
      Config.MOOD_VISUALS.EMOJIS[mood] || Config.MOOD_VISUALS.DEFAULT_EMOJI
    );
  }
  ...
  createCalendarDropdown() {
    ...
    const bg = this.add
      .rectangle(0, 0, width, collapsedHeight, 0x000000, 0.7)
      .setOrigin(0, 0)
    ...
    const contentContainer = this.add
      .container(0, collapsedHeight)
      .setVisible(false);
    const contentBg = this.add
      .rectangle(0, 0, width, expandedHeight - collapsedHeight, 0x222222, 0.9)
      .setOrigin(0, 0)
    ...
    bg.on("pointerdown", () => {
      ...
      bg.setSize(
        width,
        container.isExpanded ? expandedHeight : collapsedHeight,
      );
  ...
  updateCalendarDropdown(worldState) {
    ...
    const timeIcon =
      timePeriod === "Night" || timePeriod === "Dusk" || timePeriod === "Dawn"
        ? "🌙"
        : "☀️";
  ...
  createModal(title) {
    ...
    const modalBg = this.add
      .rectangle(0, 0, w, h, 0x1a1a1a, 0.95)
      .setStrokeStyle(2, 0xffffff)
      .setInteractive();
    ...
    const modalContent = this.add
      ...
      .setOrigin(0.5);
    const closeButton = ButtonFactory.createButton(
      this,
      w / 2 - 40,
      -h / 2 + 30,
      "X",
  ...
  openCraftingMenu() {
    ...
    if (this.craftingDynamicItems) {
      this.craftingDynamicItems.forEach((i) => i.destroy());
    }
    this.craftingDynamicItems = [];

    container.content.setText("");

    const items = Object.entries(this.nadagotchiData.inventory).filter(
      ([k, v]) => v > 0,
    );
    let startY = -150;

    const invTitle = this.add.text(-200, -180, "Inventory:", {
      fontSize: "18px",
      fontFamily: "VT323",
    });
    ...
    for (let i = 0; i < 9; i++) {
      ...
      const slot = this.add
        .rectangle(x, y, cellSize, cellSize, 0x333333)
    ...
    const craftBtn = ButtonFactory.createButton(
      this,
      180,
      0,
      "CRAFT!",
      () => {
        const mainScene = this.scene.get("MainScene");
        const result = mainScene.nadagotchi.inventorySystem.craftFromGrid(
          this.craftingGridState,
        );
  ...
  showDialogue(npcName, dialogueData) {
    ...
    if (typeof dialogueData === "string") {
      ...
      options = dialogueData.options || [{ label: "Close", action: null }];
    }
    if (this.dialogueButtons) {
      this.dialogueButtons.forEach((btn) => btn.destroy());
    }
    ...
    options.forEach((opt) => {
      const btn = ButtonFactory.createButton(
        ...
        () => {
          ...
          if (opt.action) {
            if (opt.action === "GIFT_CALLBACK") {
              this.game.events.emit(
                EventKeys.UI_ACTION,
                EventKeys.INTERACT_NPC,
                { npc: npcName, type: "GIFT" },
              );
            } else {
              opt.action();
            }
  ...
  async openJournal() {
    this.closeAllModals();
    const entries = await this.persistence.loadJournal();
    const modal = this.journalModal;
    modal.entries = entries.slice().reverse();
    modal.currentPage = 0;
    ...
    if (!modal.navButtons) {
      ...
      const yPos = mh / 2 - 40;
      modal.btnPrev = ButtonFactory.createButton(
        this,
        -80,
        yPos,
        "< Prev",
        () => this.changeJournalPage(-1),
        { width: 80, height: 30 },
      );
      modal.btnNext = ButtonFactory.createButton(
        this,
        80,
        yPos,
        "Next >",
      ...
      modal.pageIndicator = this.add
        .text(0, yPos, "1/1", { fontFamily: "VT323", fontSize: "20px" })
        .setOrigin(0.5);
      modal.add([modal.btnPrev, modal.btnNext, modal.pageIndicator]);
      modal.navButtons = true;
    }
  ...
  changeJournalPage(delta) {
    const modal = this.journalModal;
    const totalPages =
      Math.ceil(modal.entries.length / modal.entriesPerPage) || 1;
  ...
  updateJournalPage() {
    ...
    const pageEntries = modal.entries.slice(
      start,
      start + modal.entriesPerPage,
    );
    const text = pageEntries.length
      ? pageEntries.map((e) => `[${e.date}]\n${e.text}`).join("\n\n---\n\n")
      : "No entries yet.";
    ...
    startTutorial() {
        ...
        const yesBtn = ButtonFactory.createButton(this, -10, 100, "Yes", () => {
            container.destroy();
            this.runTutorialSequence();
        }, { width: 100, height: 40, color: 0x4CAF50 });

        const noBtn = ButtonFactory.createButton(this, 110, 100, "No", () => {
            container.destroy();
            this.scene.resume('MainScene');
  ...
  async openRecipeBook() {
    this.closeAllModals();
    const discovered =
      (this.nadagotchiData && this.nadagotchiData.discoveredRecipes) ||
      (await this.persistence.loadRecipes());
    const allRecipes =
      (this.nadagotchiData && this.nadagotchiData.recipes) || {};
    let text =
  ...
  openHobbyMenu() {
    this.closeAllModals();
    if (!this.nadagotchiData) return;
    this.hobbyModal.content.setText(
      Object.entries(this.nadagotchiData.hobbies)
        .map(([h, l]) => `${h}: Level ${l}`)
        .join("\n"),
    );
  ...
  openRelationshipMenu() {
    this.closeAllModals();
    if (!this.nadagotchiData) return;
    this.relationshipModal.content.setText(
      Object.entries(this.nadagotchiData.relationships)
        .map(([n, d]) => `${n}: Friendship ${d.level}`)
        .join("\n"),
    );
  ...
  openDecorateMenu() {
    ...
    const furniture = Object.entries(this.nadagotchiData.inventory).filter(
      ([item, count]) => {
    ...
    furniture.forEach(([itemName, count]) => {
      ...
      const isSurface =
        def && (def.type === "WALLPAPER" || def.type === "FLOORING");
      const actionKey = isSurface
        ? EventKeys.APPLY_HOME_DECOR
        : EventKeys.PLACE_FURNITURE;
      const btnText = isSurface ? "Apply" : "Place";
      const placeButton = ButtonFactory.createButton(
        this,
        startX,
        yPos,
        btnText,
    ...
    const toggleBtn = ButtonFactory.createButton(
      this,
      0,
      moveBtnY,
      "Move Furniture",
      () => {
        this.game.events.emit(
          EventKeys.UI_ACTION,
          EventKeys.TOGGLE_DECORATION_MODE,
        );
        this.decorateModal.setVisible(false);
        this.scene.resume("MainScene");
      },
      { width: 160, height: 40, color: 0x4169e1 },
    );
  ...
  openInventoryMenu() {
    ...
    items.forEach(([itemName, count]) => {
      ...
      const itemText = this.add.text(startX, currentY, itemStr, {
        font: "20px monospace",
        color: "#ffffff",
      });
  ...
  async openAchievementsModal() {
    this.closeAllModals();
    const data = await this.persistence.loadAchievements();
    const unlockedIds = data.unlocked || [];
    const text = Achievements.map((ach) =>
  ...
  startTutorial() {
    ...
    const container = this.add.container(width / 2, height / 2).setDepth(1000);
    const bg = this.add
      .rectangle(0, 0, 400, 250, 0x000000, 0.9)
      .setStrokeStyle(2, 0xffffff)
      .setInteractive();
    const title = this.add
      .text(0, -80, "Welcome to Nadagotchi!", {
        fontFamily: "VT323",
        fontSize: "32px",
        color: "#fff",
      })
    ...
    const content = this.add
      .text(0, -20, "Would you like a quick tour\nof the interface?", {
        fontFamily: "VT323",
        fontSize: "24px",
        color: "#fff",
        align: "center",
    ...
    const yesBtn = ButtonFactory.createButton(
      this,
      -60,
      80,
      "Yes",
    ...
    const noBtn = ButtonFactory.createButton(
      this,
      60,
      80,
      "No",
  ...
  runTutorialSequence() {
    ...
    const msgBg = this.add
      .rectangle(0, 0, 500, 120, 0x333333, 0.9)
      .setStrokeStyle(2, 0xffffff);
    ...
    const hintText = this.add
      .text(0, 40, "(Click anywhere to continue)", {
        fontFamily: "VT323",
        fontSize: "18px",
        color: "#aaa",
      })
  ...
  showToast(title, message, icon = "") {
    this.toastManager.show({ title, message, icon, style: "GOLD" });
  }
  createSettingsModal() {
    ...
    const volLabel = this.add
      ...
      .setOrigin(0.5);
    const volDown = ButtonFactory.createButton(
      this,
      -80,
      -40,
      "-",
    ...
    const volUp = ButtonFactory.createButton(
      this,
      80,
      -40,
      "+",
    ...
    const volDisplay = this.add
      .text(0, -40, "50%", { fontSize: "24px", fontFamily: "VT323" })
      .setOrigin(0.5);
    const speedLabel = this.add
      .text(0, 20, "Game Speed", { fontSize: "24px", fontFamily: "VT323" })
      .setOrigin(0.5);
    const speedButtons = [];
    const speeds = [
      { l: "1x", v: Config.SETTINGS.SPEED_MULTIPLIERS.NORMAL },
      { l: "2x", v: Config.SETTINGS.SPEED_MULTIPLIERS.FAST },
      { l: "5x", v: Config.SETTINGS.SPEED_MULTIPLIERS.HYPER },
    ];
    let startX = -80;
    speeds.forEach((s) => {
      const btn = ButtonFactory.createButton(
  ...
  updateSpeedButtons(speed) {
    this.settingsModal.speedButtons.forEach((btn) => {
      if (Math.abs(btn.speedVal - speed) < 0.01) {
        btn.setAlpha(1);
        btn.setScale(1.1);
      } else {
        btn.setAlpha(0.6);
  ...
  createShowcaseModal() {
    ...
    const passportContainer = this.add.container(
      this.cameras.main.width / 2,
      this.cameras.main.height / 2,
    );
  ...
  openShowcase() {
    ...
    const cardBg = this.add
      .rectangle(0, 0, width, height, 0xfff8e7)
      .setStrokeStyle(4, 0xd4af37);
    const frame =
      Config.MOOD_VISUALS.FRAMES[this.nadagotchiData.mood] ??
      Config.MOOD_VISUALS.DEFAULT_FRAME;
    const sprite = this.add.image(-120, 0, "pet", frame).setScale(8);
    ...
    const footer = this.add
      .text(0, 80, "OFFICIAL NADAGOTCHI PASSPORT", {
        fontFamily: "Arial",
        fontSize: "12px",
        color: "#888888",
        fontStyle: "italic",
  ...
  openCareerMenu() {
    ...
    if (career) {
      ...
      const title = CareerDefinitions.TITLES[career]
        ? CareerDefinitions.TITLES[career][level]
        : "Employee";
    ...
    const listTitle = this.add
      .text(0, yPos, "Unlocked Career Paths:", {
        fontFamily: "VT323",
        fontSize: "20px",
        color: "#AAAAAA",
      })
    ...
    if (unlocked.length === 0) {
      ...
      unlocked.forEach((c) => {
        ...
        const rowText = this.add.text(-mw / 2 + 60, yPos + 10, label, {
          fontFamily: "VT323",
          fontSize: "24px",
        ...
        if (!isCurrent) {
          const switchBtn = ButtonFactory.createButton(
            this,
            mw / 2 - 80,
            yPos + 20,
            "Switch",
          ...
          const activeLbl = this.add.text(mw / 2 - 80, yPos + 10, "[Active]", {
            fontFamily: "VT323",
            fontSize: "18px",
            color: "#00FF00",
          });
  ...
  openJobBoardMenu() {
    ...
    if (this.jobBoardDynamicItems) {
      this.jobBoardDynamicItems.forEach((i) => i.destroy());
    }
    ...
    const startBtn = ButtonFactory.createButton(
      ...
      },
      {
        width: 160,
        height: 50,
        color: career ? 0x6a0dad : 0x555555,
        fontSize: "24px",
      },
    );
    ...
    const careerBtn = ButtonFactory.createButton(
      this,
      0,
      100,
      "Career Profiles",
