...
//     ];
    ...
    createSettingsModal() {
        ...
        let startX = -80;
        speeds.forEach(s => { const btn = ButtonFactory.createButton(this, startX + 30, 80, s.l, () => { this.game.events.emit(EventKeys.UPDATE_SETTINGS, { gameSpeed: s.v }); this.updateSpeedButtons(s.v); if (!this.settingsData) this.settingsData = {}; this.settingsData.gameSpeed = s.v; }, { width: 60, height: 40, fontSize: '20px', color: 0x008080 }); btn.speedVal = s.v; speedButtons.push(btn); startX += 80; });
    ...
    updateSpeedButtons(speed) {
        this.settingsModal.speedButtons.forEach(btn => { if (Math.abs(btn.speedVal - speed) < 0.01) { btn.setAlpha(1); btn.setScale(1.1); } else { btn.setAlpha(0.6); btn.setScale(1.0); } });
    }
    ...
    openShowcase() {
        ...
        const width = 400; const height = 250;
        const cardBg = this.add.rectangle(0, 0, width, height, 0xFFF8E7).setStrokeStyle(4, 0xD4AF37);
        const frame = Config.MOOD_VISUALS.FRAMES[this.nadagotchiData.mood] ?? Config.MOOD_VISUALS.DEFAULT_FRAME;
        ...
        const infoText = this.add.text(0, -60, `${name}\n${gen}\n${career}\n${age}`, { fontFamily: 'VT323, monospace', fontSize: '24px', color: '#000000', lineSpacing: 10 }).setOrigin(0, 0);
        const footer = this.add.text(0, 80, "OFFICIAL NADAGOTCHI PASSPORT", { fontFamily: 'Arial', fontSize: '12px', color: '#888888', fontStyle: 'italic' }).setOrigin(0.5);
    ...
    openCareerMenu() {
        ...
        if (this.careerDynamicItems) { this.careerDynamicItems.forEach(i => i.destroy()); }
        ...
        const statsText = this.add.text(0, yPos, infoText, { fontFamily: 'VT323, monospace', fontSize: '24px', color: '#ffffff', align: 'center', wordWrap: { width: mw - 60 } }).setOrigin(0.5, 0);
        container.add(statsText);
        this.careerDynamicItems.push(statsText);
        yPos += 140;
        const listTitle = this.add.text(0, yPos, "Unlocked Career Paths:", { fontFamily: 'VT323', fontSize: '20px', color: '#AAAAAA' }).setOrigin(0.5);
        ...
        if (unlocked.length === 0) {
            ...
            unlocked.forEach(c => {
                ...
                if (!isCurrent) {
                    const switchBtn = ButtonFactory.createButton(this, (mw/2 - 80) + 40, (yPos + 20) + 15, "Switch", () => { this.game.events.emit(EventKeys.UI_ACTION, EventKeys.SWITCH_CAREER, c); container.setVisible(false); this.scene.resume('MainScene'); }, { width: 80, height: 30, color: 0x4CAF50, fontSize: '18px' });
    ...
    openJobBoardMenu() {
        ...
        const infoText = this.add.text(0, -50, text, { fontFamily: 'VT323', fontSize: '32px', color: '#FFF', align: 'center' }).setOrigin(0.5);
